...
{
    /// <summary>Checks that the data is valid for the handler</summary>
    bool IsValid(ReadOnlySpan<byte> data, string? ext);

    /// <summary>Extracts the range of bytes that represent the CMS blob</summary>
    ReadOnlySpan<byte> ExtractSignature(ReadOnlySpan<byte> data);

    /// <summary>Computes a hash of the data as defined by the signing specification.</summary>
    byte[] ComputeHash(ReadOnlySpan<byte> data, HashAlgorithmName hashAlgorithm);
    ...
    long RemoveSignature(Span<byte> data);

    /// <summary>Writes the encoded CMS structure into a signature structure.</summary>
    void WriteSignature(IAllocation allocation, Signature signature);
    ...
    Signature CreateSignature(ReadOnlySpan<byte> data, HashAlgorithmName hashAlgorithm);
    ...
    bool TryGetHash(SignedCms signedCms, [NotNullWhen(true)]out byte[]? digest, out HashAlgorithmName algo);
}